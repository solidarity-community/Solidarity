...
import { Account, AccountService } from 'sdk'
...
export class DialogAuthentication extends DialogAuthenticator<Account> {
	...
	protected async authenticateAccount() {
		await AccountService.authenticateWithPassword(this.username, this.password)
		const account = await AccountService.getAuthenticated()
		return account as Account
	}

	protected unauthenticateAccount() {
		return Promise.resolve(AccountService.unauthenticate())
	}

	protected getAuthenticatedAccount() {
		return AccountService.getAuthenticated()
	}

	private async register() {
		await new DialogAccountRegister().confirm()
		return await AccountService.getAuthenticated()
	}
