...
import { CampaignMedia, CampaignMediaType, type Campaign, type ButtonFileUpload } from 'application'
...
export class DialogCampaignMedia extends DialogComponent<{ readonly campaign?: Campaign }, CampaignMedia> {
	...
	private get selectionTemplate() {
		return html`
			<mo-flex direction='horizontal' gap='10px'>
				<mo-button type='outlined' @click=${() => this.mediaType = CampaignMediaType.File}>Image file</mo-button>
				<mo-button type='outlined' @click=${() => this.mediaType = CampaignMediaType.YouTube}>YouTube video</mo-button>
				<mo-button type='outlined' @click=${() => this.mediaType = CampaignMediaType.Twitch}>Twitch video</mo-button>
			</mo-flex>
		`
	}
	...
	protected override async primaryAction() {
		...
		return {
			type: this.mediaType,
			uri: CampaignMedia.extractUriByType(this.content, this.mediaType),
		}
	}
}