...
public sealed class CampaignFundingModule : Module
{
	...
	{
		endpoints.MapGet("/api/campaign/{id}/balance", [AllowAnonymous] async (IDatabase database, IPaymentMethodProvider paymentMethodProvider, int id) => (await database.Get<Campaign>(id)).GetTotalBalance(paymentMethodProvider));
